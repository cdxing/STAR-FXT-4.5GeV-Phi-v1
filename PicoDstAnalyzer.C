...
//////////////////////////////// Main Function /////////////////////////////////
void PicoDstAnalyzer3(const Char_t *inFile = "./files/PicoDst/st_physics_16140033_raw_0000002.picoDst.root",
                    ...
                    )
{
  ...
  // Flow Histograms
  TProfile3D *profile3D_proton_v1 = new TProfile3D("profile3D_proton_v1","Proton v_{1}",Ncentralities,0.5,Ncentralities+0.5,ptBins,ptLow,ptHigh,rapidityBins,rapidityLow,rapidityHigh,"");
  ...
  TProfile3D *profile3D_pionPlus_v1 = new TProfile3D("profile3D_pionPlus_v1","#pi^{#plus} v_{1}",Ncentralities,0.5,Ncentralities+0.5,ptBins,ptLow,ptHigh,rapidityBins,rapidityLow,rapidityHigh,"");
  ...
  TProfile3D *profile3D_pionMinus_v1 = new TProfile3D("profile3D_pionMinus_v1","#pi^{#minus} v_{1}",Ncentralities,0.5,Ncentralities+0.5,ptBins,ptLow,ptHigh,rapidityBins,rapidityLow,rapidityHigh,"");
  ...
  TProfile3D *profile3D_kaonPlus_v1 = new TProfile3D("profile3D_kaonPlus_v1","K^{#plus} v_{1}",Ncentralities,0.5,Ncentralities+0.5,ptBins,ptLow,ptHigh,rapidityBins,rapidityLow,rapidityHigh,"");
  ...
  TProfile3D *profile3D_kaonMinus_v1 = new TProfile3D("profile3D_kaonMinus_v1","K^{#minus} v_{1}",Ncentralities,0.5,Ncentralities+0.5,ptBins,ptLow,ptHigh,rapidityBins,rapidityLow,rapidityHigh,"");
  ...
  TH2D  *h2_phi_v1_vs_invM = new TH2D("h2_phi_v1_vs_invM","h2_phi_v1_vs_invM",100,0.9,1.1,1000,-1.0,1.0);
  h2_phi_v1_vs_invM->GetXaxis()->SetTitle("K^{+}K^{-} Invariant Mass(GeV/c^{2})");
  h2_phi_v1_vs_invM->GetYaxis()->SetTitle("v_{1}");

  TH2D  *h2_phi_v1_vs_invM_bin2 = new TH2D("h2_phi_v1_vs_invM_bin2","h2_phi_v1_vs_invM_bin2, -1.5 <= y < -1.0",100,0.9,1.1,1000,-1.0,1.0);
  h2_phi_v1_vs_invM_bin2->GetXaxis()->SetTitle("K^{+}K^{-} Invariant Mass(GeV/c^{2})");
  h2_phi_v1_vs_invM_bin2->GetYaxis()->SetTitle("v_{1}");

  TH2D  *h2_phi_v1_vs_invM_bin3 = new TH2D("h2_phi_v1_vs_invM_bin3","h2_phi_v1_vs_invM_bin3, -1.0 <= y < -0.5",100,0.9,1.1,1000,-1.0,1.0);
  h2_phi_v1_vs_invM_bin3->GetXaxis()->SetTitle("K^{+}K^{-} Invariant Mass(GeV/c^{2})");
  h2_phi_v1_vs_invM_bin3->GetYaxis()->SetTitle("v_{1}");

  TH2D  *h2_phi_v1_vs_invM_bin4 = new TH2D("h2_phi_v1_vs_invM_bin4","h2_phi_v1_vs_invM_bin4, -0.5 <= y <= 0.0",100,0.9,1.1,1000,-1.0,1.0);
  h2_phi_v1_vs_invM_bin4->GetXaxis()->SetTitle("K^{+}K^{-} Invariant Mass(GeV/c^{2})");
  h2_phi_v1_vs_invM_bin4->GetYaxis()->SetTitle("v_{1}");

  TProfile  *TP_phi_v1_vs_invM      = nullptr;
  TProfile  *TP_phi_v1_vs_invM_bin2 = nullptr;
  TProfile  *TP_phi_v1_vs_invM_bin3 = nullptr;
  TProfile  *TP_phi_v1_vs_invM_bin4 = nullptr;
  ...
  // TPC EPs
  TH1D *hist_tpc_east_psi_raw = new TH1D("hist_tpc_east_psi_raw","TPC east EP",500,-1.5*TMath::Pi(),2.5*TMath::Pi());
  hist_tpc_east_psi_raw->GetXaxis()->SetTitle("#psi^{TPC east}_{1} [Radian]");
  hist_tpc_east_psi_raw->GetYaxis()->SetTitle("# of events");

  TH1D *hist_tpc_east_psi_recentered = new TH1D("hist_tpc_east_psi_recentered","TPC east EP",500,-1.5*TMath::Pi(),2.5*TMath::Pi());
  ...
  TH1D *hist_tpc_east_psi_flattened = new TH1D("hist_tpc_east_psi_flattened","TPC east EP",500,-1.5*TMath::Pi(),2.5*TMath::Pi());
  ...
  TFile *eventPlanes_input   = new TFile("/star/data01/pwg/dchen/Ana/fxtPicoAna/result_systematic_error_analysis_directed_flow/phi_meson_directed_flow_primary.root","read");
...
cout<<KaonPlusEfficiencyTable<<endl;
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++)
  {
    ...
    if(N_tpc_east >= 5 && tpc_east_Qweight > 0.0)
    {
      ...
      if(tpc_east_Qx || tpc_east_Qy)
      {
        ...
        if(tpc_east_Qx || tpc_east_Qy)
        {
          ...
          for(Int_t k=0;k<order;k++)
          {
            reaction_plane_new += ( -2.0*psi_mean[1+2*k] * TMath::Cos( 1.*tpc_east_plane2*(k+1) )
                                   + 2.0*psi_mean[0+2*k] * TMath::Sin( 1.*tpc_east_plane2*(k+1) ) ) / (k+1);
          }
    ...
    if( tpc_east_plane3 >= 0.0 && tpc_east_plane3 <= 2.0*TMath::Pi() )
    {
      if( tpc_west_plane3 >= 0.0 && tpc_west_plane3 <= 2.0*TMath::Pi() )
      {
        profile_correlation_tpc_east_tpc_west->Fill(centrality,TMath::Cos(tpc_east_plane3 - tpc_west_plane3));
        correlation2D_tpc_sub->Fill(tpc_west_plane3,tpc_east_plane3);
      }
      if( bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() )
      {
        profile_correlation_tpc_east_bbc_east->Fill(centrality,TMath::Cos(tpc_east_plane3 - bbc_east_plane3 - TMath::Pi()));
    ...
    if( tpc_west_plane3 >= 0.0 && tpc_west_plane3 <= 2.0*TMath::Pi() )
    {
      if( bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() )
      {
        profile_correlation_tpc_west_bbc_east->Fill(centrality,TMath::Cos(tpc_west_plane3 - bbc_east_plane3 - TMath::Pi()));
    ...
    Double_t d_resolution2 = TMath::Cos(tpc_west_plane3 - bbc_east_plane3 - TMath::Pi())*TMath::Cos(tpc_east_plane3 - bbc_east_plane3 - TMath::Pi())/TMath::Cos(tpc_east_plane3 - tpc_west_plane3);
    ...
    for(Int_t itr=0;itr<nTracks;itr++)
    {
      ...
      )
      {
        ...
        // Accumulate flows
        if( /*chooseBBCeastEP &&*/ bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() /*&& d_resolution[centrality-1]>0*//*&& efficiency > 0.0 */) {
            profile3D_proton_v1->Fill(centrality,pt,rap_Proton/*track->eta()*/,TMath::Cos(phi - bbc_east_plane3 - TMath::Pi())/*/d_resolution[centrality-1]*/,/*efficiency*/1.0);
        }
      ...
      )
      {
        ...
        if(charge > 0)
        {
          ...
          // Accumulate flows
          if( /*chooseBBCeastEP &&*/ bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() && efficiency > 0.0  /*&& d_resolution[centrality-1]>0*/) {
              profile3D_pionPlus_v1->Fill(centrality,pt,rap_Pion/*track->eta()*/,TMath::Cos(phi - bbc_east_plane3 - TMath::Pi())/*/d_resolution[centrality-1]*/,efficiency/*1.0*/);
          }
        ...
        if(charge < 0)
        {
          ...
          // Accumulate flows
          if( /*chooseBBCeastEP &&*/ bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() && efficiency > 0.0 /*&& d_resolution[centrality-1]>0*/) {
              profile3D_pionMinus_v1->Fill(centrality,pt,rap_Pion/*track->eta()*/,TMath::Cos(phi - bbc_east_plane3 - TMath::Pi())/*/d_resolution[centrality-1]*/,efficiency/*1.0*/);
          }
      ...
      )
      {
        ...
        if(charge > 0)
        {
          ...
          // Accumulate flows
          if( /*chooseBBCeastEP &&*/ bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() && efficiency > 0.0 /*&& d_resolution[centrality-1]>0*/) {
              profile3D_kaonPlus_v1->Fill(centrality,pt,rap_Kaon/*track->eta()*/,TMath::Cos(phi - bbc_east_plane3 - TMath::Pi())/*/d_resolution[centrality-1]*/,efficiency/*1.0*/);
          }
        ...
        if(charge < 0)
        {
          ...
          // Accumulate flows
          if( /*chooseBBCeastEP &&*/ bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() && efficiency > 0.0 /*&& d_resolution[centrality-1]>0*/) {
              profile3D_kaonMinus_v1->Fill(centrality,pt,rap_Kaon/*track->eta()*/,TMath::Cos(phi - bbc_east_plane3 - TMath::Pi())/*/d_resolution[centrality-1]*/,efficiency/*1.0*/);
          }
    ...
    for(int i = 0; i < v_pri_tracks0.size();i++)
    {
      ...
      for(int j = 0; j < v_pri_tracks1.size(); j++)
      {
        ...
        double d_v1_raw_phi = -999.0;
        if(  bbc_east_plane3 >= 0.0 && bbc_east_plane3 <= 2.0*TMath::Pi() ) {
           d_v1_raw_phi = TMath::Cos(d_phi_azimuth - bbc_east_plane3 - TMath::Pi());
        }

        if(/*b_cent_01||b_cent_02||*/b_cent_07) continue;
        if(d_v1_raw_phi == -999.0) continue;
        // std::cout<< d_v1_raw_phi <<std::endl;
        // if(b_cent_03) d_v1_raw_phi /= 0.398419;
        // if(b_cent_04) d_v1_raw_phi /= 0.454645;
        // if(b_cent_05) d_v1_raw_phi /= 0.498949;
        // if(b_cent_06) d_v1_raw_phi /= 0.596082;
        if(b_cent_01) d_v1_raw_phi /= d_resolution[0];
        if(b_cent_02) d_v1_raw_phi /= d_resolution[1];
        if(b_cent_03) d_v1_raw_phi /= d_resolution[2];
        if(b_cent_04) d_v1_raw_phi /= d_resolution[3];
        if(b_cent_05) d_v1_raw_phi /= d_resolution[4];
        if(b_cent_06) d_v1_raw_phi /= d_resolution[5];
        // std::cout<< d_v1_raw_phi <<std::endl;

        h2_phi_v1_vs_invM                                        -> Fill(d_inv_m,d_v1_raw_phi,d_eff_corr0*d_eff_corr1);
        if(d_phi_y>=-1.5 && d_phi_y<-1.0) h2_phi_v1_vs_invM_bin2 -> Fill(d_inv_m,d_v1_raw_phi,d_eff_corr0*d_eff_corr1);
        if(d_phi_y>=-1.0 && d_phi_y<-0.5) h2_phi_v1_vs_invM_bin3 -> Fill(d_inv_m,d_v1_raw_phi,d_eff_corr0*d_eff_corr1);
        if(d_phi_y>=-0.5 && d_phi_y<=0.0) h2_phi_v1_vs_invM_bin4 -> Fill(d_inv_m,d_v1_raw_phi,d_eff_corr0*d_eff_corr1);
  ...
  profile3D_proton_v1->Write();
  profile3D_pionPlus_v1->Write();
  profile3D_pionMinus_v1->Write();
  profile3D_kaonPlus_v1->Write();
  profile3D_kaonMinus_v1->Write();

  h_K_DCA_r      ->Write();
  h_K_obj_DCA_r  ->Write();
  h_K_diff_DCA_r ->Write();
  h_prim_inv_m_PHI ->Write();
  h2_phi_v1_vs_invM->Write();
  h2_phi_v1_vs_invM_bin2->Write();
  h2_phi_v1_vs_invM_bin3->Write();
  h2_phi_v1_vs_invM_bin4->Write();

  TP_phi_v1_vs_invM      = h2_phi_v1_vs_invM->ProfileX();
  TP_phi_v1_vs_invM_bin2 = h2_phi_v1_vs_invM_bin2->ProfileX();
  TP_phi_v1_vs_invM_bin3 = h2_phi_v1_vs_invM_bin3->ProfileX();
  TP_phi_v1_vs_invM_bin4 = h2_phi_v1_vs_invM_bin4->ProfileX();

  TP_phi_v1_vs_invM     ->Write();
  TP_phi_v1_vs_invM_bin2->Write();
  TP_phi_v1_vs_invM_bin3->Write();
  TP_phi_v1_vs_invM_bin4->Write();
